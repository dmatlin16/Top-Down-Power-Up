class Barrier:
    ...
    def near_point(self, x, y, d = 7):
        ...
        # Get equation of barrier (y = mx + b), m is slope, b is y-intercept
        m = (y2 - y1) / (x2 - x1)
        b = y1 - m*x1
        
        # Get equations of lines perpendicular to barrier and going through (x1, y1) and (x2, y2)
        if y1 == y2:
            y2 += 0.0001
        m_perp = -(x2 - x1) / (y2 - y1)
        ...
        # If the point is between these perpendicular lines, find how close point is to barrier 
        len_sqr = (x2 - x1)**2 + (y2 - y1)**2
        d1_perp_sqr = (m_perp*x + b1_perp - y)**2 / (m_perp**2 + 1)
        d2_perp_sqr = (m_perp*x + b2_perp - y)**2 / (m_perp**2 + 1)
        
        if d1_perp_sqr <= len_sqr and d2_perp_sqr <= len_sqr:
            # Get distance from point to line, squared - returns true if dist_sqr < d^2
            dist_sqr = (m*x + b - y)**2 / (m**2 + 1)
            return dist_sqr <= d**2
        return False