...
def setup():
    ...
    for variant in img_variants:
        ...
        switch_img = loadImage("../Assets/Images/Field/Switch Variants/" + variant + ".png")
        # switch_img.resize(int(scale_factor * switch_img.width / 2), int(scale_factor * switch_img.height / 2))
        switch_img.resize(switch_img.width / 2, switch_img.height / 2)
        switch_imgs[variant] = switch_img
        
        # Load scale image of that variant
        scale_img = loadImage("../Assets/Images/Field/Scale Variants/" + variant + ".png")
        # scale_img.resize(int(scale_factor * scale_img.width / 2), int(scale_factor * scale_img.height / 2))
        scale_img.resize(scale_img.width / 2, scale_img.height / 2)
        scale_imgs[variant] = scale_img
